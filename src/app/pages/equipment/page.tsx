...
import {RefreshCw, Package, Search, X, ChevronDown, Eye, Calendar, FileText, Tag} from 'lucide-react';
import {useEquipment} from '@/hooks/useEquipment';
import {EquipmentView} from '@/types/type';

export default function EquipmentPage() {

    const {equipments, applyFilters, fetchEquipments, fetchEquipmentById, loading, error} = useEquipment();
    ...
    // ✅ เช็คว่าควรแสดงคอลัมน์ไหน
    const showLicenseKey = selectedType === 'all' || selectedType === '1';
    /**
     * โดยค่าของมัน (showLicenseKey) จะเป็น
     * true ถ้า selectedType มีค่าเท่ากับ 'all' หรือ '1'
     * false ถ้าไม่ใช่ทั้งสองอย่าง
     **/
    const showSerialNumber = selectedType === 'all' || selectedType === '2';

    // ✅ ฟังก์ชัน Filter หลายเงื่อนไขพร้อมกัน
    const handleApplyFilters = async () => {
        await applyFilters({
            ...
            /**
             * “ฟังก์ชัน handleApplyFilters ทำหน้าที่รวบรวมค่าที่ผู้ใช้เลือกในหน้าเว็บ เช่น ประเภท สถานะ และคำค้นหา
             * จากนั้นส่งต่อไปยังฟังก์ชัน applyFilters() ซึ่งจะเรียก API ของ backend เพื่อกรองข้อมูลอุปกรณ์จากฐานข้อมูล
             * โดยถ้าผู้ใช้ไม่ได้เลือกบางเงื่อนไข (เช่นเลือก ‘ทั้งหมด’) ระบบจะไม่ส่งพารามิเตอร์ส่วนนั้น (ใช้ undefined แทน) เพื่อให้ backend ไม่กรองในส่วนนั้น
             * **/
        });
    };
    ...
    // ✅ เรียก API เมื่อ dropdown เปลี่ยน
    useEffect(() => {
        handleApplyFilters();
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                {/* Header */}
                <div
                    className="mb-6 bg-white rounded-xl shadow-lg p-6 border-l-4 border-indigo-600 flex justify-between items-center">
                    <div>
                ...
                <div className="bg-white rounded-xl shadow-md overflow-hidden">
                    {/* ✅ แปลง Ternary เป็น if-else แบบเข้าใจง่าย */}
                    {(() => {
                        // ถ้ากำลังโหลด
                        if (loading) {
                            return (
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full">
                                    ...
                                    <tr>
                                        ...
                                        {showSerialNumber && (
                                            <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">Serial
                                                Number</th>
                                        )}

                                        {showLicenseKey && (
                                            <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">License
                                                Key</th>
                                        )}
                                    ...
                                    {equipments.map((eq, i) => (
                                        <tr key={eq.id}
                                            className={`hover:bg-indigo-50 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                                            ...
                                            {showLicenseKey && (
                                                <td className="px-6 py-3">
                                                    {eq.licenseKey ? (
                                                        <span
                                                            className="font-mono text-sm text-purple-900 bg-purple-50 px-2 py-1 rounded">
                                                            {eq.licenseKey}
                                                        </span>
                                                    ) : (
                                                        <span className="text-gray-400">-</span>
                                            ...
                                            <td className="px-6 py-3">
                                                <button
                                                    onClick={() => handleViewDetail(eq.id)}
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto">
            ...
            <div className="flex min-h-screen  items-center justify-center p-4">
                <div
                    className="relative bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-300">
                    ...
                    {(() => {
                        ...
                        return (
                            <>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            {equipment.serialNumber && (
                                                <InfoItem label="Serial Number">
                                                    <span
                                                        className="font-mono text-sm text-gray-900 bg-gray-100 px-3 py-1 rounded">
                                                        {equipment.serialNumber}
                                                    </span>
                                                </InfoItem>
                                            )}
                                            ...
                                            {equipment.licenseKey && (
                                                <InfoItem label="License Key">
                                                    <span
                                                        className="font-mono text-sm text-purple-900 bg-purple-50 px-3 py-1 rounded">
                                                        {equipment.licenseKey}
                                                    </span>
                                                </InfoItem>
                                            )}
                                ...
                                {/* Footer */}
                                <div
                                    className="sticky bottom-0 bg-gray-50 border-t border-gray-200 p-4 flex justify-end">
                                    <button
                                        onClick={onClose}
...
// ✅ Component สำหรับแสดงข้อมูลแต่ละรายการ
function InfoItem({
                      label,
                      value,
                      children

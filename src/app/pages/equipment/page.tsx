...
import { RefreshCw, Package, Search, X, ChevronDown } from 'lucide-react';
import { useEquipment } from '@/hooks/useEquipment';

export default function EquipmentPage() {
    const { equipments, applyFilters, fetchEquipments, loading, error } = useEquipment();
    ...
    // ✅ เรียก API เมื่อ dropdown เปลี่ยน (ยกเว้น searchTerm)
    useEffect(() => {
        handleApplyFilters();
            ...
            <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className="mb-6 bg-white rounded-xl shadow-lg p-6 border-l-4 border-indigo-600 flex justify-between items-center">
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 flex items-center gap-2 font-medium disabled:opacity-50 transition-colors"
                    >
                    ...
                    <div className="mb-6">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-semibold disabled:opacity-50 transition-colors flex items-center gap-2 whitespace-nowrap"
                            >
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-bold uppercase tracking-wider">License Key</th>
                                ...
                                {equipments.map((eq, i) => (
                                    <tr key={eq.id} className={`hover:bg-indigo-50 transition-colors ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                                        ...
                                        <td className="px-6 py-3">
                                            {eq.licenseKey ? (
                                                <span className="font-mono text-sm text-purple-900 bg-purple-50 px-2 py-1 rounded">
                                                        {eq.licenseKey}
                                                    </span>
                                            ) : (
                                                <span className="text-gray-400">-</span>
